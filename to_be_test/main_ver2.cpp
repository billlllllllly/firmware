...
#define PLAYER_NUM_CTRL_PIN_3 21
#define PLAYER_NUM_CTRL_PIN_2 22
#define PLAYER_NUM_CTRL_PIN_1 26 // pullup -> high->0
#define PLAYER_NUM_CTRL_PIN_0 27 // four pins from left to right, representing the binary numo of player num, left as MSB
...
void msg(const String& s, int sz = 2) {
    ...
    oled.setTextSize(sz);
    oled.setCursor(1, 1);
    oled.print("Player Num: ");
    oled.print(PLAYER_NUM);
    oled.print(", ");
    oled.println(readings);
    oled.println(s);
    oled.display();
}
...
void readPlayerNum() {
    int b0 = !digitalRead(PLAYER_NUM_CTRL_PIN_0); 
    int b1 = !digitalRead(PLAYER_NUM_CTRL_PIN_1); 
    int b2 = !digitalRead(PLAYER_NUM_CTRL_PIN_2); 
    int b3 = !digitalRead(PLAYER_NUM_CTRL_PIN_3);
    readings = (b3 << 3) | (b2 << 2) | (b1 << 1) | b0;
...
void setup() {
    ...
    pinMode(PLAYER_NUM_CTRL_PIN_0, INPUT_PULLUP);
    pinMode(PLAYER_NUM_CTRL_PIN_1, INPUT_PULLUP);
    pinMode(PLAYER_NUM_CTRL_PIN_2, INPUT_PULLUP);
    pinMode(PLAYER_NUM_CTRL_PIN_3, INPUT_PULLUP);
    readPlayerNum();
    ...
    // readingslate arg is the data GPIO (must be a literal). Length must match
